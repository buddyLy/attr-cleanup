...
def log_timer(msg, start_time, end_time):
...
def get_frequent_value_old(similarvaluelist):
    """ Return the value that appears the most in the list """
    global numFreqValComp
    current_value = None
    valuetouse = None
    ...
    for line in sys.stdin:
        ...
        for myline in lines:
            ...
            for word in words:
                ...
                end_time = time.time()
                log_timer("create_similarvalue_list", start_time, end_time)
                
                #get the most frequently appeared value
                start_time = time.time()
                attrvalue = get_frequent_value(similarValueList)
                end_time = time.time()
                log_timer("get_frequent_value", start_time, end_time)
                ...
                end_time = time.time()
                log_timer("isValueCompared", start_time, end_time)

				#TODO: make attrvaluelist as a set, then instead of adding each individual words, add the attr value of it, which would reduce the number of comparisons
            ...
            log_timer("Processing totalword [%s] for key [%s]" % (totalcount,key), line_starttime, line_endtime)
...
numSimComp [%s] \t\t numFreqValComp [%s] \t\t numCalcDistComp [%s]\n" % (round(createSimList_totaltime,2), round(getFreqVal_totaltime,2), round(isValComp_totaltime,2), numSimComp, numFreqValComp, numCalcDistComp), start_time, end_time)
